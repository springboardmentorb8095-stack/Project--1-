...
function FreelancerDashboard() {
  ...
  // New states for profile menu and delete popup
  ...
  // New function for toggling profile menu
  const toggleProfileMenu = () => {
    setShowProfileMenu(!showProfileMenu);
  };

  // New function for handling account deletion
  const handleDeleteAccount = () => {
    if (confirmInput.toLowerCase() === "delete") {
      ...
      // Clear notifications and other data if needed
      setNotifications([]);
  ...
  // Handle status change for accepted projects
  ...
  // Auto-scroll notification list to bottom when shown
  useEffect(() => {
      ...
      <main className="main-content">
              ...
              {appliedProjects.map((app, index) => (
                ...
                >
                  ...
                  {app.status === "Accepted" &&
                    app.projectStatus === "Completed" &&
                    app.clientRating > 0 && (
                      <p>Client Rating: {app.clientRating} ⭐</p>
                    )}
                </li>
              ))}
            </ul>
        ...
        {showDeletePopup && (
          <div className="popup">
            ...
            <button onClick={() => { setShowDeletePopup(false); setConfirmInput(""); }}>Cancel</button>
          </div>
        )}
      </main>
    </div>
