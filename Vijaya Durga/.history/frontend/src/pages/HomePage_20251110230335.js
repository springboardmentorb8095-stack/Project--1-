...
function HomePage() {
  ...
  // 🧠 Typing Animation (Slow & Limited per Word)
  useEffect(() => {
    ...
    const handleScroll = () => {
      titles.forEach((title) => {
        ...
        if (rect.top < window.innerHeight - 100 && !title.classList.contains("typed")) {
          title.classList.add("typed");
          const text = title.textContent;
          const speed = 90; // ✨ slow natural speed
          typeText(title, text, speed);
        }
      });
    };
